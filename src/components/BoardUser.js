...
const BoardUser = () => {
  ...
  const getMyRecipes = () => {
    ...
    axios.get(`${API_BASE_URL}api/users/getUserRecipes/${currentUser.id}`)
    .then(res=>{
  ...
  const createRecipe = () => {
    ...
    //console.log(state);
    axios.post(`${API_BASE_URL}api/recipes`, state)
      .then(res=>{
        console.log(res.data);
  ...
  const updateRecipeList = (e)=> {
    ...
    setIsImageUploaded(true)
    axios.get(`${API_BASE_URL}api/recipes/${e.target.id}`)
    .then(res=>{
  ...
  const removeRecipeFunc = () => {
    axios.delete(`${API_BASE_URL}api/recipes/${updateRecipe._id}`)
    .then(res=>{
      console.log(res.data)
  ...
  const updateRecipeFunc = () => {
    axios.put(`${API_BASE_URL}api/recipes/${updateRecipe._id}`, state)
    .then(res=>{
      console.log(res.data);
      dispatch({type: "clearAfterCreate", payload: ""});
      setRecipeClicked(false);
